...
class TestBasher(unittest.TestCase):
    ...
    def test_cmd(self, mock_run):
        ...
        result = self.bash.cmd("ls -la", show_output=False)
        self.assertEqual(result, "Command output")
    ...
    def test_write_to_file(self, mock_cmd):
        """Test the write_to_file function."""
        # Test write mode
        self.bash.write_to_file("/test/file.txt", "Test content")
        mock_cmd.assert_called_with("echo 'Test content' > '/test/file.txt'")

        # Test append mode
        self.bash.write_to_file("/test/file.txt", "More content", 'a')
        mock_cmd.assert_called_with("echo 'More content' >> '/test/file.txt'")

        # Test with invalid mode
        with self.assertRaises(ValueError):
            self.bash.write_to_file("/test/file.txt", "Content", 'x')

    @patch('basher.file_ops.FileOps.cmd')
    @patch('basher.file_ops.FileOps.exists')
    def test_read_file(self, mock_exists, mock_cmd):
        """Test the read_file function."""
        mock_exists.return_value = True
        mock_cmd.return_value = "File content"

        # Test with existing file
        result = self.bash.read_file("/test/file.txt")
        mock_cmd.assert_called_with("cat '/test/file.txt'", show_output=False)
    ...
    def test_replace_in_file(self, mock_isfile, mock_exists, mock_cmd):
        ...
        # The command should be called with the properly escaped pattern and replacement
    ...
    def test_detect_package_manager(self, mock_cmd):
        ...
        # Test with apt
        mock_cmd.return_value = True  # which apt returns success
        ...
        def side_effect(cmd, **kwargs):
            if cmd == "which apt":
                return False  # which apt returns failure
            elif cmd == "which yum":
                return True   # which yum returns success
            return False
        ...
        mock_cmd.side_effect = lambda cmd, **kwargs: False  # All commands fail
        result = self.bash.detect_package_manager()
        self.assertIsNone(result)
    ...
    def test_archive(self, mock_isdir, mock_folder_exists, mock_exists, mock_run):
        ...
        self.bash.archive_ops.system = mock_system
